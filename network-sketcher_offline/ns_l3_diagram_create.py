...
class  ns_l3_diagram_create():
    def __init__(self):
        ...
        if self.click_value == 'L3-3-2':
            ...
            if self.click_value_l3 == 'L3-4-1':
                ...
                # 2nd pass; bumps calculated_max_right_edge so slide_width
                # adjusts.
            ...
            # because l3_area_create mutates their inner lists.
            ...
            # Runaway-device pull-back + area separation. A device connected by a
            # long L3 segment can land far to the right of its area's device
            # cluster (e.g. SRV_* at the end of a cross-diagram segment), making
            # the area frame overrun and overlap the next area's frame. We pull
            # such a device back to just right of its cluster, then push any
            # following area right so the (now larger) frames no longer overlap.
            # Both shifts go through device_extra_shift and re-render. Runs
            # before the segment lane-separation retry so the latter operates on
            # the final X positions.
    ...
    def l3_area_create(self, target_folder_name, action_type,offset_x ,offset_y):
                        ...
                        if current_if_key not in drawn_no_l3seg_interfaces:
                            ...
                            if action_type == 'CREATE' and not minimal_1st_pass:
                                ...
                                for ip_entry in all_ip_entries_for_if:
                                    ...
                                    self.shape = self.slide.shapes
                                    ns_ddx_figure.extended.add_shape(self, ip_entry[0], tag_ip_left_2, ip_y, tag_ip_width, ip_entry[4], ip_addr)
        '''
        ...
        # analysis. Only record once during the 1st pass.
        if action_type == 'CREATE' and self.click_value_l3 == 'L3-4-1' and self.flag_re_create == False:
...
def get_optimize_y_grid_array(self):
    # Global lane packing: pick smallest k such that lane Y =
    # first_shape_top + k * 0.25 has no X overlap with any segment ALREADY
    # placed at that Y across ALL groups. This catches cross-group collisions
    # the previous per-group algorithm missed.
...
def _detect_l3_overspread(self, threshold=3.0):
    ...
    # practice, while the multi-row misalignment cascade spreads a row by
    # 30-44". A 3.0" threshold cleanly separates the two, so the conditional
    # 4th pass only fires on the pathological case and leaves all other
    # diagrams untouched.
...
def compute_l3_device_shifts(self):
    ...
    # that shares a Y-band (side-by-side; expansion separates them in X).
    ...
    # clear THAT area's devices. Devices then pack tightly behind the first
    # one past the block. Constraint A holds within the area, and across
    # Y-bands stacked-area blockers are kept (see the Y-band check below).
    ...
    def _per_area_cumul(row, shifts_self, shifts_iter=None):
        # Per-device cumulative shift, resetting at each area boundary so it
        # matches the actual draw-loop behavior (get_l3_shape_offset resets
        # left_offset to area_start_x at the first device of each area row).
        out = {}
        acc = 0.0
        last_area = None
        for d in row:
    ...
    for area_name, lines in self._collision_lines_per_area.items():
        ...
        # Per real-area top Y for Y-band grouping (see same_area_only above).
                ...
                for d in row:
                    ...
                    for line in d_lines:
                        ...
                        for blocker_name, b in devs.items():
                            if blocker_name == d:
                                continue
                            # 4th pass: ignore a different-area blocker only
                            # when it shares a Y-band with the device's area
                            # (side-by-side; expansion separates them in X).
                            # Stacked-area blockers (different Y-band) are kept.
                            if same_area_only:
    ...
    # moved it past the block. Pull each device LEFT to the leftmost position
    # whose connector still clears all relevant blockers (same Y-band cross-
    # area blockers ignored, identical to the fixpoint rule) and that keeps
    # >= pitch from its left neighbour. Decrease-only, so constraint A holds.
    ...
    # Per-area expansion propagation: when devices in area A shift right,
    # subsequent areas IN THE SAME HORIZONTAL Y-BAND shift right by the
    # cumulative expansion of all prior areas in that band to avoid frame
    # overlap. Areas in different Y-bands are independent.
...
def _detect_and_fix_2nd_pass_overlaps(self):
    ...
    # y_grid_segment_array. When found, push the LATER segment's optimize Y
    # down by 0.25" so the next CREATE pass places it on the next lane.
    # Returns True if any overlaps were detected (re-render needed).
...
def _pull_back_runaway_devices(self, gap_threshold=2.0):
    ...
    # and makes it overlap the neighbouring area. Here we find, per area, the
    # first clean empty band (no device of any row reaches into it) wider than
    # `gap_threshold` inches and pull the entire right group - and everything
    # further right - left so the band shrinks to one normal column gap.
    ...
    # width, so it converges. A single runaway device is just a right group of
    # size one, so this generalises - and preserves - the previous single-device
    # behaviour (it lands one column to the right of the left group).
    ...
    # area_name -> { device_name: [min_left, max_right, top] } aggregated over
    # every shape carrying that device's name (icon + tags), matching the shapes
    # the drawn area frame is built from.
    area_devs = {}
    for sh in write_arr:
...
def _separate_overlapping_areas(self, area_margin_x=0.5, area_margin_y=0.34, desired_gap=0.5):
    ...
    # frame is reproduced exactly as it is drawn: bounding box of every shape in
    # the area (devices + segment bars + labels) expanded by area_margin_x /
    # area_margin_y, with '_wp_' areas excluded (they draw no frame). Returns
    # the largest per-area shift applied this round (0.0 when nothing was
    # shifted); the caller uses the magnitude to detect divergence.
    write_arr = getattr(self, 'add_shape_write_array', None)
    uad = getattr(self, 'unique_area_device_array', [])
    if not write_arr or not uad:
        return 0.0
...
def _restore_state_and_rerender(self):
    ...
    # re-run the CREATE pass with the updated optimize_y_grid_array.
    # position_shape_array / position_folder_array are deepcopied because
    # l3_area_create mutates their inner lists.
    # IMPORTANT: do NOT `from pptx import Presentation` locally here -- the
    # SVG path monkeypatches the module-level Presentation with
    # _MockPresentation. A local import bypasses the mock and breaks SVG
    # 3rd pass rendering (TypeError: expected lxml.etree._Element). Use
    # the module-level `Presentation` instead.
    import copy
