...
class  nsm_l1_diagram_create():
    def __init__(self):
        ...
        self.position_line_array = nsm_def.convert_master_to_array(ws_name, ppt_meta_file, '<<POSITION_LINE>>')
        self.position_style_shape_array = nsm_def.convert_master_to_array(ws_name, ppt_meta_file, '<<STYLE_SHAPE>>')
        self.position_tag_array = nsm_def.convert_master_to_array(ws_name, ppt_meta_file, '<<POSITION_TAG>>')
        ...
        _ps_max_row = max((r for r, _c in self.position_shape_tuple.keys()), default=0)
        for tmp_wp_folder_name in folder_wp_name_array[1]:
            ...
            while flag_end_row == False:
                if current_row > _ps_max_row:
                    flag_end_row = True
                    break
                _cell_val = str(self.position_shape_tuple.get((current_row, 1), ''))
                if _cell_val == tmp_wp_folder_name:
                    start_row = current_row
                    flag_start_row = True
                if flag_start_row == True and _cell_val == '<END>':
                    flag_end_row = True
            ...
            for tmp_folder_name in folder_wp_name_array[0]:
                ...
                while flag_end_row == False:
                    if current_row > _ps_max_row:
                        flag_end_row = True
                        break
                    _cell_val2 = str(self.position_shape_tuple.get((current_row, 1), ''))
                    if _cell_val2 == tmp_folder_name:
                        start_row = current_row
                        flag_start_row = True
                    if flag_start_row == True and _cell_val2 == '<END>':
                        flag_end_row = True
                        end_row = current_row - 1
                    current_row += 1
                ...
                # The section marker row in the _tmp_ sheet is preserved
                # automatically by _write_excel_meta_xlsx via insert_rows().
                # Adding (1,1) breaks the offset_row compression and produces
                # a blank row 2 in _tmp_, which terminates add_sub_folder()
                # early and yields an empty PPTX (only outer frame drawn).
                ...
                #### GET best width size ####
                master_folder_size_array = nsm_def.get_folder_width_size(master_folder_tuple, master_style_shape_tuple, master_shape_tuple, min_tag_inches)
                ...
                nsm_def.write_excel_meta(master_root_folder_tuple, excel_file_path, worksheet_name, write_to_section, offset_row, offset_column)
                ...
                if self.click_value == '2-4-1':
                    ...
                    nsm_def.clear_tag_in_position_line(tmp_ws_name, ppt_meta_file, clear_section_tuple)
        ...
        if self.click_value == '2-4-3' or self.click_value == '2-4-4' or self.click_value == 'VPN-1-1':
            # copy Master_Data sheet to _tmp_
            nsm_def.copy_excel_sheet(ws_name, ppt_meta_file, tmp_ws_name)

            if self.click_value == '2-4-3' or self.click_value == 'VPN-1-1':
                #### clear <<POSITION_TAG>>
                clear_section_tuple = self.position_tag_tuple
                clear_section_tuple[1, 1] = '<<POSITION_TAG>>'
                nsm_def.clear_section_sheet(tmp_ws_name, ppt_meta_file, clear_section_tuple)
            ...
            min_tag_inches = 0.3
            master_folder_size_array = nsm_def.get_folder_width_size(self.position_folder_tuple, self.position_style_shape_tuple, self.position_shape_tuple, min_tag_inches)
