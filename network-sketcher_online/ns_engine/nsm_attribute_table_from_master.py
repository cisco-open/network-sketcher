...
import nsm_def, nsm_egt_maker, openpyxl
from openpyxl.styles import Font, Color
import ast

class  nsm_attribute_table_from_master():
    def __init__(self):
        ...
        self.attribute_array = nsm_def.convert_master_to_array('Master_Data', excel_maseter_file, '<<ATTRIBUTE>>')
        ...
        get_shape_folder_tuple = nsm_def.get_shape_folder_tuple(self.position_shape_tuple)
        ...
        attribute_master_data_tuple = nsm_def.convert_array_to_tuple(remake_attribute_table_array)
        ...
        master_is_nsm = str(excel_maseter_file).lower().endswith('.nsm')
        if master_is_nsm:
            input_excel_name = nsm_def._get_staging_xlsx(excel_maseter_file)
        else:
            ...
            for i, tmp_tmp_color_grid_rgb_array in enumerate(tmp_color_grid_rgb_array[1],start=0):
                if i >= 3 and tmp_tmp_color_grid_rgb_array != '<END>':
                    if i >= 3:
                        ...
                        nsm_def.change_cell_color(self.input_tree_excel, sheet_name, row, column, rgb_color)

        ''' Apply color to title cells that can be changed '''
        row = 2
        for column in range(3, 12):
            nsm_def.change_cell_color(self.input_tree_excel, sheet_name, row, column, [246,252,252])
