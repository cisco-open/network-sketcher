...
import nsm_def ,nsm_ddx_figure , network_sketcher,ns_dev
...
def l2_device_table_sync_with_l3_master(self):
    ...
    original_master_l2_table_array = nsm_def.convert_master_to_array(ws_l2_name, excel_maseter_file, '<<L2_TABLE>>')
    original_master_l3_table_array = nsm_def.convert_master_to_array(ws_l3_name, excel_maseter_file, '<<L3_TABLE>>')
    ...
    nsm_def.remove_excel_sheet(ppt_meta_file, copy_sheet_name)
    ...
    l3_table_path = self.full_filepath.replace('[MASTER]', '[L3_TABLE]')
    if l3_table_path.lower().endswith('.nsm'):
        l3_table_path = l3_table_path[:-4] + '.xlsx'
    if os.path.isfile(l3_table_path) and nsm_def.check_file_locked(l3_table_path) == False:
        os.remove(l3_table_path)

    ### get re-create L3 Master
    recreate_master_l3_table_array = nsm_def.convert_master_to_array(ws_l3_name, excel_maseter_file, '<<L3_TABLE>>')
    ...
    for index_l3_2 ,tmp_recreate_master_l3_table_array in enumerate(recreate_master_l3_table_array):
        tmp_recreate_master_l3_table_array[1].append(nsm_def.get_if_value(tmp_recreate_master_l3_table_array[1][2]))
        tmp_recreate_master_l3_table_array[1].extend([str(nsm_def.split_portname(tmp_recreate_master_l3_table_array[1][2])[0])])
    ...
    ### Re-Write L3 sheet
    last_master_l3_table_tuple = nsm_def.convert_array_to_tuple(last_master_l3_table_array)
    tmp_ws_name = ws_l3_name
    ppt_meta_file = excel_maseter_file
    clear_section_taple = last_master_l3_table_tuple
    nsm_def.clear_section_sheet(tmp_ws_name, ppt_meta_file, clear_section_taple)
...
def l2_device_port_name_sync_with_l3_master(self):
    ...
    folder_wp_name_array = nsm_def.get_folder_wp_array_from_master(ws_name, excel_maseter_file)
    ...
    self.device_l2_table_array = nsm_def.convert_excel_to_array(excel_ws_name, excel_file, start_row)
    ...
    master_l2_table_array = nsm_def.convert_master_to_array(ws_l2_name, excel_maseter_file, '<<L2_TABLE>>')
    master_l3_table_array = nsm_def.convert_master_to_array(ws_l3_name, excel_maseter_file, '<<L3_TABLE>>')
    ...
    #convert to tuple
    master_l2_table_tuple = nsm_def.convert_array_to_tuple(master_l2_table_array)
    master_l3_table_tuple = nsm_def.convert_array_to_tuple(master_l3_table_array)
...
def l1_master_device_and_line_sync_with_l2l3_master(self):
    ...
    nsm_def.remove_excel_sheet(excel_maseter_file, ws_l2_name)
    nsm_def.remove_excel_sheet(excel_maseter_file, ws_l3_name)

    import nsm_l2_table_from_master
    self.inFileTxt_L2_1_1.delete(0, tk.END)
    self.inFileTxt_L2_1_1.insert(tk.END, excel_maseter_file)

    nsm_l2_table_from_master.nsm_l2_table_from_master.__init__(self)

    # ========== STEP 4: Load re-created L2 data ==========
    recreate_master_l2_table_array = nsm_def.convert_master_to_array(ws_l2_name, excel_maseter_file, '<<L2_TABLE>>')
    ...
    for recreate_item in recreate_master_l2_table_array:
        ...
        if key in original_l2_with_port:
            ...
            if_value = nsm_def.get_if_value(recreate_item[1][3])
            portname = str(nsm_def.split_portname(recreate_item[1][3])[0])
    ...
    last_master_l2_table_tuple = nsm_def.convert_array_to_tuple(last_l2_table_array)
    nsm_def.clear_section_sheet(ws_l2_name, excel_maseter_file, last_master_l2_table_tuple)
    nsm_def.overwrite_excel_meta(last_master_l2_table_tuple, excel_maseter_file, ws_l2_name, '<<L2_TABLE>>', 0, 0)
    ...
    for recreate_item in recreate_master_l3_table_array:
        ...
        if key in original_l3_dict:
            ...
            # Use re-created data (new entry)
            if_value = nsm_def.get_if_value(recreate_item[1][2])
            portname = str(nsm_def.split_portname(recreate_item[1][2])[0])
    ...
    last_master_l3_table_tuple = nsm_def.convert_array_to_tuple(last_l3_table_array)
    nsm_def.clear_section_sheet(ws_l3_name, excel_maseter_file, last_master_l3_table_tuple)
    nsm_def.overwrite_excel_meta(last_master_l3_table_tuple, excel_maseter_file, ws_l3_name, '<<L3_TABLE>>', 0, 0)
