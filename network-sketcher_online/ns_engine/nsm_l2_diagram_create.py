...
def is_multiprocessing_supported():
    ...
    Disabled when running inside the web server (NS_WEB_SERVER env var)
    to avoid console window flicker and stdout capture conflicts.
    """
    if os.environ.get('NS_WEB_SERVER'):
        return False
        ...
        class CompleteContext:
            ...
            def _ensure_l2_shared_prepared(self):
                ...
                if (not hasattr(self, "l2_shared_data_prepared") or not self.l2_shared_data_prepared
                    ...
                    nsm_ddx_figure.ns_ddx_figure_run.prepare_l2_shared_data(self)
...
def process_device_worker(device_name):
    ...
    try:
        ...
        device_size_array = nsm_ddx_figure.extended.l2_device_materials(
...
class nsm_l2_diagram_create():
    ...
    def __init__(self):
        ...
        try:
            ...
            self.position_folder_array = nsm_def.convert_master_to_array(ws_name, excel_maseter_file, '<<POSITION_FOLDER>>')
            self.position_shape_array = nsm_def.convert_master_to_array(ws_name, excel_maseter_file, '<<POSITION_SHAPE>>')
            self.position_line_array = nsm_def.convert_master_to_array(ws_name, excel_maseter_file, '<<POSITION_LINE>>')
            self.position_style_shape_array = nsm_def.convert_master_to_array(ws_name, excel_maseter_file, '<<STYLE_SHAPE>>')
            self.position_tag_array = nsm_def.convert_master_to_array(ws_name, excel_maseter_file, '<<POSITION_TAG>>')
            self.root_folder_array = nsm_def.convert_master_to_array(ws_name, excel_maseter_file, '<<ROOT_FOLDER>>')
            ...
            self.l2_table_array = nsm_def.convert_master_to_array(ws_l2_name, excel_maseter_file, '<<L2_TABLE>>')
            ...
            if self.click_value == 'L2-3-2':
                ...
                data_array = []
                nsm_l2_diagram_create.l2_area_create(self, action_type, data_array)
            ...
            if self.click_value == 'L2-3-3':
                ...
                for idx, tmp_all_device_size_array in enumerate(self.all_device_l2_size_array, 1):
                    ...
                    try:
                        nsm_ddx_figure.extended.l2_device_materials(
    ...
    def l2_area_create(self, action_type, data_array):
        ...
        l2_position_style_shape_tuple = nsm_def.convert_array_to_tuple(l2_position_style_shape_array)
        ...
        nsm_def.copy_excel_sheet(ws_name, ppt_meta_file, tmp_ws_name)

        clear_section_tuple = self.position_folder_tuple
        nsm_def.clear_section_sheet(tmp_ws_name, ppt_meta_file, clear_section_tuple)
        ...
        if action_type == 'CREATE_L2_AREA':
            nsm_def.write_excel_meta(convert_tuple, ppt_meta_file, tmp_ws_name, '<<POSITION_FOLDER>>', 0, 0)

        if action_type == 'CREATE_L2_AREA':
            nsm_def.clear_section_sheet(tmp_ws_name, ppt_meta_file, self.position_style_shape_tuple)

        nsm_def.write_excel_meta(l2_position_style_shape_tuple, ppt_meta_file, tmp_ws_name, '<<STYLE_SHAPE>>', 0, 0)
        ...
        master_root_folder_tuple = nsm_def.get_root_folder_tuple(self, master_folder_size_array, tmp_folder_name)

        clear_section_tuple = dict(master_root_folder_tuple)
        clear_section_tuple[1, 1] = '<<ROOT_FOLDER>>'
        if action_type == 'CREATE_L2_AREA':
            nsm_def.clear_section_sheet(tmp_ws_name, ppt_meta_file, clear_section_tuple)
            nsm_def.write_excel_meta(master_root_folder_tuple, ppt_meta_file, tmp_ws_name, '<<ROOT_FOLDER>>', 0, 0)

        import copy
        update_current_y_grid_array = copy.deepcopy(current_y_grid_array)

        tmp_sheet_position_folder_array = nsm_def.convert_master_to_array(tmp_ws_name, ppt_meta_file, '<<POSITION_FOLDER>>')
        tmp_sheet_position_folder_tuple = nsm_def.convert_array_to_tuple(tmp_sheet_position_folder_array)
        ...
        update_current_y_grid_tuple = nsm_def.convert_array_to_tuple(update_current_y_grid_array)

        clear_sheet_position_folder_array = nsm_def.convert_master_to_array(tmp_ws_name, ppt_meta_file, '<<POSITION_FOLDER>>')
        clear_sheet_position_folder_tuple = nsm_def.convert_array_to_tuple(clear_sheet_position_folder_array)
        nsm_def.clear_section_sheet(tmp_ws_name, ppt_meta_file, clear_sheet_position_folder_tuple)
        ...
        if action_type == 'CREATE_L2_AREA':
            nsm_def.write_excel_meta(update_current_y_grid_tuple, ppt_meta_file, tmp_ws_name, '<<POSITION_FOLDER>>', 0, 0)
        ...
        if str(ppt_meta_file).lower().endswith('.nsm'):
            self.excel_file_path = nsm_def._get_staging_xlsx(ppt_meta_file)
        else:
        ...
        if action_type == 'CREATE_L2_AREA':
            ...
            nsm_ddx_figure.ns_ddx_figure_run.__init__(self)
