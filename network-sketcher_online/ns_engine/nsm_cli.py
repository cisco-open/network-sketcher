...
class ns_cli_run():
    ...
    def cli_export(self, master_file_path, argv_array):
        ...
        Type 'YES' to proceed or 'NO' to cancel: '''
                ...
                try:
                    ...
                    nsm_extensions.ai_context.export_ai_context_file(self, 'dummy')
                except Exception as e:
                    import traceback
        ...
        if next_arg == 'device_file':
            try:
                ...
                if os.path.isfile(device_file_path):
                    ...
                    if nsm_def.check_file_open(device_file_path):
                ...
                try:
                    import nsm_l1_table_from_master
                    nsm_l1_table_from_master.nsm_l1_table_from_master.__init__(self)
                except Exception as e:
                    dummy_tk.destroy()
                    import traceback
                    error_detail = traceback.format_exc()
                ...
                try:
                    ...
                    if str(master_file_path).lower().endswith('.nsm'):
                        from ns_engine.nsm_io import _read_all_from_nsm
                        ws_list = ['Master_Data']
                        nsm_sections = _read_all_from_nsm(master_file_path)
                        if 'L2_TABLE' in nsm_sections:
                            ws_list.append('Master_Data_L2')
                    else:
                    ...
                    if 'Master_Data_L2' not in ws_list:
                        import nsm_l2_table_from_master
                        nsm_l2_table_from_master.nsm_l2_table_from_master.__init__(self)
                    else:
                        import nsm_l2_table_from_master
                        nsm_l2_table_from_master.ns_l2_table_from_master_l2_sheet.__init__(self)
                except Exception as e:
                    dummy_tk.destroy()
                    import traceback
                    error_detail = traceback.format_exc()
                ...
                try:
                    ...
                    if str(master_file_path).lower().endswith('.nsm'):
                        ...
                        ws_list = ['Master_Data']
                        nsm_sections = _read_all_from_nsm(master_file_path)
                        if 'L3_TABLE' in nsm_sections:
                            ws_list.append('Master_Data_L3')
                    else:
                        input_excel_master_data = openpyxl.load_workbook(master_file_path)
                        ws_list = input_excel_master_data.sheetnames
                        input_excel_master_data.close()
                    ...
                    if 'Master_Data_L3' not in ws_list:
                        import nsm_l3_table_from_master
                        nsm_l3_table_from_master.nsm_l3_table_from_master.__init__(self)
                    else:
                        import nsm_l3_table_from_master
                        nsm_l3_table_from_master.ns_l3_table_from_master_l3_sheet.__init__(self)
                except Exception as e:
                    dummy_tk.destroy()
                    import traceback
                    error_detail = traceback.format_exc()
                ...
                try:
                    ...
                    if not nsm_def.check_file_open(device_file_path):
                        import nsm_attribute_table_from_master
                        nsm_attribute_table_from_master.nsm_attribute_table_from_master.__init__(self)
                except Exception as e:
                    dummy_tk.destroy()
                    import traceback
                    error_detail = traceback.format_exc()
        ...
        if next_arg == 'master_file_backup':
            try:
                ...
                # Call nsm_def.get_backup_filename to create backup
                backup_file_path = nsm_def.get_backup_filename(master_file_path)
        ...
        if next_arg == 'l1_diagram':
            try:
                ...
                # --attribute argument (optional, default: first attribute)
                attribute_title_list = nsm_def.get_attribute_title_list(self, master_file_path)
                selected_attr = get_next_arg(argv_array, '--attribute')
                if selected_attr is None or selected_attr.startswith('--'):
                    selected_attr = attribute_title_list[0] if attribute_title_list else ''
                elif attribute_title_list and selected_attr not in attribute_title_list:
                    return ([f'[ERROR] Invalid --attribute value: {selected_attr}. Valid values: {", ".join(attribute_title_list)}'])

                if attribute_title_list and selected_attr:
                    self.attribute_tuple1_1 = nsm_def.get_global_attribute_tuple(master_file_path, selected_attr)
                else:
                ...
                class MockCombo:
                    ...
                    def get(self):
                        return self._value
                self.comboATTR_1_1 = MockCombo(selected_attr)

                self.click_value = click_value

                # --format argument (optional, default: pptx)
                output_format = get_next_arg(argv_array, '--format') or 'pptx'
                if output_format not in ('pptx', 'svg'):
                    output_format = 'pptx'

                if output_format == 'svg':
                    output_svg = os.path.join(iDir, prefix + basename_clean + '.svg')
                ...
                import nsm_l1_diagram_create
                nsm_l1_diagram_create.nsm_l1_diagram_create.__init__(self)
                ...
                if click_value in ('2-4-3', '2-4-4'):
                    ...
                    nsm_extensions.summary_diagram.export_summary_diagram(self, 'Dummy')
                    nsm_l1_diagram_create.nsm_l1_diagram_create.__init__(self)
                    self.flag_summary_diagram = False

                    if hasattr(self, 'excel_maseter_file_backup') and os.path.isfile(self.excel_maseter_file_backup) and not nsm_def.check_file_locked(self.excel_maseter_file_backup):
        ...
        if next_arg == 'l2_diagram':
            try:
                ...
                if attribute_title_list and len(attribute_title_list) > 0:
                    self.attribute_tuple1_1 = nsm_def.get_global_attribute_tuple(master_file_path, attribute_title_list[0])
                else:
                    self.attribute_tuple1_1 = {}
                ...
                if output_format == 'svg':
                    ...
                    import nsm_l2_svg_create
                    nsm_l2_svg_create.nsm_l2_svg_create.__init__(self)
                ...
                import nsm_l2_diagram_create
                try:
                    nsm_l2_diagram_create.nsm_l2_diagram_create.__init__(self)
        ...
        if next_arg == 'l3_diagram':
            try:
                ...
                if output_format == 'svg':
                    prefix = '[L3_DIAGRAM]AllAreas_' if l3_type == 'all_areas' else '[L3_DIAGRAM]PerArea_'
                    output_svg = os.path.join(iDir, prefix + basename_clean + '.svg')
                    self.output_svg_file = output_svg
                    self.output_format = 'svg'
                    self._l3_svg_type = l3_type
                ...
                if l3_type == 'per_area':
                    ...
                    try:
                        nsm_l3_diagram_create.nsm_l3_diagram_create.__init__(self)
                    except UnicodeEncodeError:
                        pass

                else:  # all_areas
                    output_pptx = os.path.join(iDir, '[L3_DIAGRAM]AllAreas_' + basename_clean + '.pptx')
                    master_ext = os.path.splitext(master_file_path)[1] or '.xlsx'
                    tmp_master = os.path.join(iDir, basename_without_ext.replace('[MASTER]', '__TMP__[MASTER]') + master_ext)
                    ...
                    import nsm_l3_diagram_create
                    nsm_l3_diagram_create.create_master_file_one_area.__init__(self)
    ...
    def cli_rename(self, master_file_path, argv_array):  # add at ver 2.5.4
        ...
        if next_arg == 'attribute_bulk':
            ...
            # Write to Excel
            if success_count > 0:
                attribute_tuple = nsm_def.convert_array_to_tuple(attribute_array)
                nsm_def.remove_rows_under_section('Master_Data', master_file_path, ori_attribute_tuple)
                nsm_def.write_excel_meta(attribute_tuple, master_file_path, 'Master_Data', '<<ATTRIBUTE>>', 0, 0)
        ...
        if next_arg == 'area':
            ...
            position_folder_array = nsm_def.convert_master_to_array(ws_name, ppt_meta_file, '<<POSITION_FOLDER>>')
            style_folder_array = nsm_def.convert_master_to_array(ws_name, ppt_meta_file, '<<STYLE_FOLDER>>')
            ...
            l2_table_array = nsm_def.convert_master_to_array(ws_name_l2, ppt_meta_file, '<<L2_TABLE>>')
            ws_name_l3 = 'Master_Data_L3'
            l3_table_array = nsm_def.convert_master_to_array(ws_name_l3, ppt_meta_file, '<<L3_TABLE>>')
            ...
            self.position_folder_tuple = nsm_def.convert_array_to_tuple(position_folder_array)
            self.style_folder_tuple = nsm_def.convert_array_to_tuple(style_folder_array)
            self.position_shape_tuple = nsm_def.convert_array_to_tuple(position_shape_array)
            self.l2_table_tuple = nsm_def.convert_array_to_tuple(l2_table_array)
            self.l3_table_tuple = nsm_def.convert_array_to_tuple(l3_table_array)
            ...
            nsm_def.overwrite_excel_meta(master_excel_meta, excel_file_path, worksheet_name, section_write_to, offset_row, offset_column)
        ...
        if next_arg == 'port':
            ...
            updated_name_array.append(nsm_def.adjust_portname(updated_name_array[1]))
            updated_name_array.append(nsm_def.adjust_portname(updated_name_array[2]))
            ...
            if nsm_def.get_if_value(str(updated_name_array[1])) == -1 or nsm_def.get_if_value(str(updated_name_array[2])) == -1:
                ...
                if isinstance(row, list) and len(row) == 2:
                    ...
                    if line_no not in (1, 2) and isinstance(fields, list):
                        if fields[0] == updated_name_array[0]:
                            if updated_name_array[4][1] == fields[12] and updated_name_array[4][2] == str(nsm_def.split_portname(fields[2])[1]):
                                return_text = '[ERROR] This port name is already exist.  --- ' + updated_name_array[0]  + ' ' + updated_name_array[2]
                                return ([return_text])
                            if updated_name_array[3][1] == fields[12] and updated_name_array[3][2] == str(nsm_def.split_portname(fields[2])[1]):
                        ...
                        if fields[1] == updated_name_array[0]:
                            if updated_name_array[4][1] == fields[16] and updated_name_array[4][2] == str(nsm_def.split_portname(fields[3])[1]):
                                return_text = '[ERROR] This port name is already exist.  --- ' + updated_name_array[0] + ' ' + updated_name_array[2]
                                return ([return_text])

                            if updated_name_array[3][1] == fields[16] and updated_name_array[3][2] == str(nsm_def.split_portname(fields[3])[1]):
            ...
            if flag_l1_port_name == True:
                ...
                self.position_line_tuple = nsm_def.convert_array_to_tuple(position_line_array)
                ...
                nsm_sync_between_layers.l1_device_port_name_sync_with_l2l3_master(self)
            ...
            l2_table_array = nsm_def.convert_master_to_array(ws_name, ppt_meta_file, '<<L2_TABLE>>')
            ...
            if flag_l2_port_name == True:
                ...
                l3_table_array = nsm_def.convert_master_to_array(ws_name, ppt_meta_file, '<<L3_TABLE>>')
        ...
        if next_arg == 'port_info_bulk':
            ...
            if len(updated_ports) > 0:
                ...
                position_line_tuple = nsm_def.convert_array_to_tuple(position_line_array)
        ...
        if next_arg == 'device':
            ...
            self.position_shape_tuple = nsm_def.convert_array_to_tuple(position_shape_array)
            self.position_line_tuple = nsm_def.convert_array_to_tuple(position_line_array)
            self.position_style_shape_tuple = nsm_def.convert_array_to_tuple(position_style_shape_array)
            self.position_tag_tuple = nsm_def.convert_array_to_tuple(position_tag_array)
            self.attribute_tuple = nsm_def.convert_array_to_tuple(attribute_array)
            ...
            self.full_filepath = master_file_path
            import nsm_sync_between_layers
            nsm_sync_between_layers.l1_sketch_device_name_sync_with_l2l3_master(self)
        ...
        if next_arg == 'l3_instance':
            import nsm_def
            l3_attribute_array = nsm_def.convert_master_to_array('Master_Data_L3', master_file_path,'<<L3_TABLE>>')
            ...
            last_l3_table_tuple = {}
            last_l3_table_tuple = nsm_def.convert_array_to_tuple(l3_attribute_array)

            # delete L2 Table sheet
            nsm_def.remove_excel_sheet(excel_maseter_file, excel_master_ws_name_l3)
            # create L2 Table sheet
            nsm_def.create_excel_sheet(excel_maseter_file, excel_master_ws_name_l3)
            # write tuple to excel master data
            nsm_def.write_excel_meta(last_l3_table_tuple, excel_maseter_file, excel_master_ws_name_l3, '_template_', 0,0)
    ...
    def cli_add(self, master_file_path, argv_array): # add at ver 2.5.3
        ...
        if next_arg == 'ip_address_bulk':
            ...
            if added_ips:
                ...
                nsm_def.remove_excel_sheet(master_file_path, excel_master_ws_name_l3)
                nsm_def.create_excel_sheet(master_file_path, excel_master_ws_name_l3)
                nsm_def.write_excel_meta(last_l3_table_tuple, master_file_path, excel_master_ws_name_l3, '_template_', 0, 0)
        ...
        if next_arg == 'portchannel_bulk':
            ...
            l2_attribute_array = nsm_def.convert_master_to_array('Master_Data_L2', master_file_path, '<<L2_TABLE>>')
            ...
            for entry_idx, entry in enumerate(portchannel_bulk_array):
                ...
                if nsm_def.get_if_value(portchannel_name) == -1:
            ...
            if configured_interfaces:
                ...
                last_l2_table_tuple = nsm_def.convert_array_to_tuple(l2_attribute_array)

                nsm_def.remove_excel_sheet(master_file_path, excel_master_ws_name_l2)
                nsm_def.create_excel_sheet(master_file_path, excel_master_ws_name_l2)
                nsm_def.write_excel_meta(last_l2_table_tuple, master_file_path, excel_master_ws_name_l2, '_template_', 0, 0)

                # Sync L2 changes with L3 table (preserve existing IP addresses)
                # Setup GUI elements for nsm_sync_between_layers
                dummy_tk = tk.Toplevel()
                dummy_tk.withdraw()
                ...
                import nsm_sync_between_layers
                nsm_sync_between_layers.l2_device_table_sync_with_l3_master(self)
        ...
        if next_arg == 'virtual_port_bulk':
            ...
            def sort_virtual_ports(vport_list):
                ...
                def sort_key(vport):
                    val = nsm_def.get_if_value(vport)
            ...
            for entry_idx, entry in enumerate(virtual_port_bulk_array):
                ...
                for vport in vport_list:
                    ...
                    if nsm_def.get_if_value(vport) == -1:
                    ...
                    existing_vport_values = [nsm_def.get_if_value(vp) for vp in existing_vports_per_device.get(hostname, [])]
                    target_value = nsm_def.get_if_value(vport)
            ...
            if new_rows_to_add:
                ...
                for hostname in rows_by_hostname:
                    rows_by_hostname[hostname] = sorted(
                        ...
                        key=lambda x: nsm_def.get_if_value(x['vport'])
                    )
        ...
        if next_arg == 'l1_link_bulk':
            ...
            skip_sync = '--skip_sync' in argv_array
            return def_common.add_l1_links_bulk(self, l1_link_bulk_array, master_file_path, skip_sync=skip_sync)
        ...
        if next_arg == 'device_location':
            ...
            skip_sync = '--skip_sync' in argv_array
            return def_common.update_device_location_batch(self, area_name, device_grid, master_file_path, skip_sync=skip_sync)
        ...
        if next_arg == 'l1_link':  # add at Ver 2.6.1
            ...
            # Validate port names
            if nsm_def.split_portname(from_portname)[0] == '' or nsm_def.split_portname(from_portname)[1] == '':
                ...
                return ([error_msg])
            elif nsm_def.split_portname(to_portname)[0] == '' or nsm_def.split_portname(to_portname)[1] == '':
                error_msg = f"Error: invalid to_portname '{to_portname}'. \n [Tips] When specifying the portname, be sure to use single quotes, such as 'port 0'."
                return ([error_msg])

            # Prepare port information
            from_port = str(nsm_def.adjust_portname(from_portname)[0]) + ' ' + str(nsm_def.split_portname(from_portname)[1])
            to_port = str(nsm_def.adjust_portname(to_portname)[0]) + ' ' + str(nsm_def.split_portname(to_portname)[1])
            from_full_portname = nsm_def.split_portname(from_portname)[0]
            to_full_portname = nsm_def.split_portname(to_portname)[0]
            ...
            position_folder_tuple = nsm_def.convert_array_to_tuple(position_folder_array)
            position_shape_tuple = nsm_def.convert_array_to_tuple(position_shape_array)
            style_shape_tuple = nsm_def.convert_array_to_tuple(style_shape_array)
            ...
            nsm_def.overwrite_excel_meta(update_position_folder_tuple, master_file_path, worksheet_name='Master_Data', section_write_to='<<POSITION_FOLDER>>', offset_row=0, offset_column=0)
        ...
        if next_arg == 'ip_address':
            ...
            # check IP Addresses
            if nsm_def.check_ip_format(add_ipaddress_name) != 'IPv4':
        ...
        if next_arg in ('l2_segment', 'virtual_port', 'portchannel', 'vport_l2_direct_binding', 'vport_l1if_direct_binding'):
            import nsm_def
            l2_attribute_array = nsm_def.convert_master_to_array('Master_Data_L2', master_file_path,'<<L2_TABLE>>')
            ...
            if 'l2_segment' in argv_array:
                ...
                if nsm_def.get_if_value(add_vport_name) == -1:
                ...
                for row in l2_attribute_array[2:]:
                    ...
                    if len(values) > 1 and values[1] == hostname:
                        if (len(values) > 5 and values[5] != '') and (len(values) > 3 and values[3] == '') :
                            tmp_if_value_array.append(nsm_def.get_if_value(values[5]))
                target_if_value = nsm_def.get_if_value(add_vport_name)
                from bisect import bisect_left
                ...
                if nsm_def.get_if_value(add_portchannel_name) == -1:
            ...
            # delete L2 Table sheet
            nsm_def.remove_excel_sheet(excel_maseter_file, excel_master_ws_name_l2)
            # create L2 Table sheet
            nsm_def.create_excel_sheet(excel_maseter_file, excel_master_ws_name_l2)
            # write tuple to excel master data
            nsm_def.write_excel_meta(last_l2_table_tuple, excel_maseter_file, excel_master_ws_name_l2, '_template_', 0,0)
    ...
    def cli_delete(self, master_file_path, argv_array):
        ...
        if next_arg == 'area':
            ...
            style_folder_array = nsm_def.convert_master_to_array('Master_Data', master_file_path, '<<STYLE_FOLDER>>')
            existing_areas = []
            ...
            style_folder_array = nsm_def.convert_master_to_array('Master_Data', master_file_path, '<<STYLE_FOLDER>>')
            ori_style_folder_tuple = nsm_def.convert_array_to_tuple(style_folder_array)
            ...
            # Write
            style_folder_tuple = nsm_def.convert_array_to_tuple(style_folder_array)
            nsm_def.remove_rows_under_section('Master_Data', master_file_path, ori_style_folder_tuple)
            nsm_def.write_excel_meta(style_folder_tuple, master_file_path,
                                    'Master_Data', '<<STYLE_FOLDER>>', 0, 0)

            # ========== Step 3: Delete from <<POSITION_SHAPE>> ==========
            position_shape_array = nsm_def.convert_master_to_array('Master_Data', master_file_path, '<<POSITION_SHAPE>>')
            ori_position_shape_tuple = nsm_def.convert_array_to_tuple(position_shape_array)
            ...
            # Write
            position_shape_tuple = nsm_def.convert_array_to_tuple(filtered_position_shape_array)
            nsm_def.remove_rows_under_section('Master_Data', master_file_path, ori_position_shape_tuple)
            nsm_def.write_excel_meta(position_shape_tuple, master_file_path,
            ...
            # STYLE_SHAPE
            style_shape_array = nsm_def.convert_master_to_array('Master_Data', master_file_path, '<<STYLE_SHAPE>>')
            ori_style_shape_tuple = nsm_def.convert_array_to_tuple(style_shape_array)
            ...
            style_shape_tuple = nsm_def.convert_array_to_tuple(style_shape_array)
            nsm_def.remove_rows_under_section('Master_Data', master_file_path, ori_style_shape_tuple)
            nsm_def.write_excel_meta(style_shape_tuple, master_file_path,
            ...
            position_tag_tuple = nsm_def.convert_array_to_tuple(position_tag_array)
            nsm_def.remove_rows_under_section('Master_Data', master_file_path, ori_position_tag_tuple)
            nsm_def.write_excel_meta(position_tag_tuple, master_file_path,
            ...
            attribute_tuple = nsm_def.convert_array_to_tuple(attribute_array)
            nsm_def.remove_rows_under_section('Master_Data', master_file_path, ori_attribute_tuple)
            nsm_def.write_excel_meta(attribute_tuple, master_file_path,
            ...
            # Write
            position_line_tuple = nsm_def.convert_array_to_tuple(position_line_array)
            nsm_def.remove_rows_under_section('Master_Data', master_file_path, ori_position_line_tuple)
            nsm_def.write_excel_meta(position_line_tuple, master_file_path,
            ...
            if tmp_delete_excel_name.lower().endswith('.nsm'):
                tmp_xlsx = tmp_delete_excel_name[:-4] + '.xlsx'
        ...
        if next_arg == 'l1_link':  # in cli_delete
            ...
            # Validate port name
            if nsm_def.split_portname(portname)[0] == '' or nsm_def.split_portname(portname)[1] == '':
                error_msg = f"Error: invalid portname '{portname}'. \n [Tips] When specifying the portname, be sure to use single quotes, such as 'port 0'."
                return ([error_msg])

            port = str(nsm_def.adjust_portname(portname)[0]) + ' ' + str(nsm_def.split_portname(portname)[1])
        ...
        if next_arg == 'ip_address':
            ...
            # check IP Addresses
            if nsm_def.check_ip_format(delete_ipaddress_name) != 'IPv4':
    ...
    def cli_show(self, master_file_path, argv_array):
        ...
        if next_arg in ('area', 'area_location'):
            import nsm_def
            position_folder_array = nsm_def.convert_master_to_array('Master_Data', master_file_path, '<<STYLE_FOLDER>>')
        ...
        if next_arg in ('device_interface', 'waypoint_interface', 'l1_link', 'l1_interface'):
            import nsm_def
            position_line_array = nsm_def.convert_master_to_array('Master_Data', master_file_path, '<<POSITION_LINE>>')
            position_line_tuple = nsm_def.convert_array_to_tuple(position_line_array)
            full_name_lookup = nsm_def.build_full_name_lookup(position_line_tuple)
            ...
            for item in position_line_array:
                ...
                if item[0] not in [1,2]:
                    if_full_name_src = full_name_lookup.get((item[1][0], item[1][2]), '_NO_MATCH_')
                    if_full_name_tar = full_name_lookup.get((item[1][1], item[1][3]), '_NO_MATCH_')
        ...
        if next_arg in ('device', 'waypoint'):
            if next_arg == 'device':
                ...
                # Auto-add ATTRIBUTE section for pre-v2.3 master files
                if attribute_array and attribute_array[0] == '_NOT_FOUND_':
                    attribute_array = nsm_def._auto_add_attribute_section(master_file_path)
        ...
        if next_arg == 'area_device':
            import nsm_def
            l2_table_array = nsm_def.convert_master_to_array('Master_Data_L2', master_file_path,   '<<L2_TABLE>>')
            new_l2_table_array = []
        ...
        if next_arg == 'l3_interface':
            import nsm_def
            l3_interface_array = nsm_def.convert_master_to_array('Master_Data_L3', master_file_path,'<<L3_TABLE>>')
        ...
        if next_arg in ('l2_broadcast_domain', 'l3_broadcast_domain'):
            import nsm_def
            return_get_l2_broadcast_domains = []
            result_get_l2_broadcast_domains = nsm_def.get_l2_broadcast_domains.run(self, master_file_path)  ## '[0] self.update_l2_table_array, [1] device_l2_boradcast_domain_array, [2] device_l2_directly_l3vport_array, [3] device_l2_other_array, [4] marged_l2_broadcast_group_array'
        ...
        if next_arg == 'attribute':
            import nsm_def
            l2_attribute_array = nsm_def.convert_master_to_array('Master_Data', master_file_path, '<<ATTRIBUTE>>')

            # Detect corrupted ATTRIBUTE data (section tag or column headers missing)
            _is_attr_corrupted = (
                l2_attribute_array and
                l2_attribute_array[0] != '_NOT_FOUND_' and
                (len(l2_attribute_array) < 2 or
                 not (isinstance(l2_attribute_array[0], list) and
                      len(l2_attribute_array[0]) >= 2 and
                      l2_attribute_array[0][1] == ['<<ATTRIBUTE>>']) or
                 not (isinstance(l2_attribute_array[1], list) and
                      len(l2_attribute_array[1]) >= 2 and
                      isinstance(l2_attribute_array[1][1], list) and
                      l2_attribute_array[1][1][0] == 'Device Name'))
            )
            if _is_attr_corrupted:
                import sys as _sys
                _sys.stderr.write('--- show attribute: Corrupted ATTRIBUTE detected, regenerating ---\n')
                l2_attribute_array = ['_NOT_FOUND_', 1]

            # Auto-add ATTRIBUTE section for pre-v2.3 master files (or corrupted files)
            if l2_attribute_array and l2_attribute_array[0] == '_NOT_FOUND_':
                l2_attribute_array = nsm_def._auto_add_attribute_section(master_file_path)

            update_attribute_array = []
            for tmp_attribute_array in l2_attribute_array:
                if not isinstance(tmp_attribute_array, list) or len(tmp_attribute_array) < 2:
                    continue
...
class def_common():
    ...
    def process_l1_link_common_batch(self, position_shape_array, style_shape_array, position_line_array,
        ...
        def update_shape_dimensions(style_shape_array, hostname, position_shape_array, position_line_array,
            ...
            text_dimensions = nsm_def.get_description_width_hight(shae_font_size, hostname)
    ...
    @staticmethod
    def add_l1_links_bulk(self, link_definitions, master_file_path, skip_sync=False):
        """
        ★★★ HIGH-PERFORMANCE VERSION ★★★

        Optimisations vs previous version:
        - Bulk file loading (1 open instead of 12+)
        - O(1) port duplicate check via dict instead of O(N) scan
        - Direction cache to avoid recomputing for same device pairs
        - Merged Step 4/5 to avoid scanning all links twice per device
        - Step 6/7 bulk file loading
        - skip_sync: when True, skip L2/L3 sync (caller handles it)
        """
        import nsm_def
        import time
        ...
        try:
            ...
            # ==================== STEP 1: Load all arrays (BULK) ====================
            print('[Step 1/8] Loading data...')
            step_start = time.time()

            bulk_sections = [
                '<<POSITION_FOLDER>>', '<<POSITION_SHAPE>>',
            ...
            style_shape_array = bulk['<<STYLE_SHAPE>>']
            position_line_array = bulk['<<POSITION_LINE>>']
            position_tag_array = bulk['<<POSITION_TAG>>']
            ori_position_line_tuple = nsm_def.convert_array_to_tuple(position_line_array)

            # Build hostname position index once
            hostname_position = {}
            for row_idx, item in enumerate(position_shape_array):
                if len(item) >= 2 and isinstance(item[1], list):
                    for col_idx, hostname in enumerate(item[1]):
            ...
            # Build existing-port set from position_line_array for O(1) duplicate check
            existing_ports = set()
            for item in position_line_array[2:]:
                if len(item) < 2 or not isinstance(item[1], list) or len(item[1]) < 4:
                    continue
                rd = item[1]
                existing_ports.add((rd[0], rd[2]))
                existing_ports.add((rd[1], rd[3]))

            # Build style_shape index for O(1) device lookup
            style_shape_index = {}
            for idx, item in enumerate(style_shape_array):
                if len(item) >= 2 and isinstance(item[1], list) and len(item[1]) > 0:
                    style_shape_index[item[1][0]] = idx

            # Direction cache: avoid recomputing for same device pairs
            direction_cache = {}

            def cached_direction(from_h, to_h):
                key = (from_h, to_h)
                ...
                direction_cache[key] = d
                direction_cache[(to_h, from_h)] = d
                return d

            batch_used_ports = {}

            def add_link_to_array(pla, from_h, to_h, from_p, to_p, direction, from_full, to_full):
                number = max([item[0] for item in pla], default=2) + 1
                if direction == 'RIGHT_LEFT':
                    sides = ['RIGHT', 'LEFT']
                elif direction == 'LEFT_RIGHT':
                    sides = ['LEFT', 'RIGHT']
            ...
            for link_idx, link_def in enumerate(link_definitions):
                ...
                from_split = nsm_def.split_portname(from_pn)
                to_split = nsm_def.split_portname(to_pn)
                ...
                from_adj = nsm_def.adjust_portname(from_pn)
                to_adj = nsm_def.adjust_portname(to_pn)
                ...
                result_dir = cached_direction(from_h, to_h)
                ...
                if (from_h, from_p) in existing_ports or (to_h, to_p) in existing_ports:
                    error_links.append({'idx': link_idx, 'err': 'Port used'})
                    continue

                if (from_h, from_p) in batch_used_ports:
                    error_links.append({'idx': link_idx, 'err': f"Conflicts with Link {batch_used_ports[(from_h, from_p)] + 1}"})
                    continue
                if (to_h, to_p) in batch_used_ports:
                    error_links.append({'idx': link_idx, 'err': f"Conflicts with Link {batch_used_ports[(to_h, to_p)] + 1}"})
                    continue
                ...
                existing_ports.add((from_h, from_p))
                existing_ports.add((to_h, to_p))
            ...
            # ==================== STEP 4+5: Calculate offsets + Update shapes (MERGED) ====================
            print(f'[Step 4/8] Calculating offsets + updating shapes for {len(all_affected_hostnames)} devices...')
            step_start = time.time()
            ...
            # Build per-device link index: {hostname: [item_ref, ...]}
            device_links = {}
            for item in position_line_array[2:]:
                cd = item[1]
                h0, h1 = cd[0], cd[1]
                if h0 in all_affected_hostnames:
                    device_links.setdefault(h0, []).append(item)
                if h1 in all_affected_hostnames:
                    device_links.setdefault(h1, []).append(item)
            ...
            for hostname in sorted(all_affected_hostnames):
                dir_groups = {'UP': [], 'DOWN': [], 'LEFT': [], 'RIGHT': []}
                up = down = left = right = 0

                for item in device_links.get(hostname, []):
                    cd = item[1]
                    fs = cd[4] if len(cd) > 4 else ''
                    ts = cd[5] if len(cd) > 5 else ''

                    if fs in ['RIGHT', 'LEFT'] and ts in ['RIGHT', 'LEFT']:
                        conn_dir = 'RIGHT_LEFT' if fs == 'RIGHT' else 'LEFT_RIGHT'
                    else:
                        conn_dir = cached_direction(cd[0], cd[1])
                ...
                # Update shapes (merged from old Step 5)
                dev_idx = style_shape_index.get(hostname)
                if dev_idx is None:
                    continue

                dev_data = style_shape_array[dev_idx][1]
                is_wp = len(dev_data) > 4 and dev_data[4] == 'BLUE'

                text_dims = nsm_def.get_description_width_hight(font_sz, hostname)
            ...
            # Update tags
            pl_tuple = nsm_def.convert_array_to_tuple(position_line_array)
            tag_off_inch = 0.02

            tag_index = {}
            for item in position_tag_array:
                if len(item) >= 2 and isinstance(item[1], list) and len(item[1]) >= 5:
                    tag_index[item[1][0]] = item
            ...
            for hostname in all_affected_hostnames:
                ...
                tag_item = tag_index.get(hostname)
                if tag_item:
                    curr = float(tag_item[1][4]) if tag_item[1][4] != '' else 0.0
                    if curr < off_val:
                        tag_item[1][4] = off_val

            print(f'[Step 5/8] (merged with Step 4)')

            # ==================== STEP 6: Write ALL sections in ONE file open ====================
            print('[Step 6/8] Writing all sections (BULK)...')
            step_start = time.time()

            nsm_def.write_sections_bulk(master_file_path, {
                '<<POSITION_LINE>>': position_line_array,
                '<<STYLE_SHAPE>>': style_shape_array,
                '<<POSITION_TAG>>': position_tag_array,
            })

            # ==================== STEP 7: Update POSITION_FOLDER ====================
            print('[Step 7/8] Updating POSITION_FOLDER...')

            bulk7 = nsm_def.convert_master_to_arrays_bulk('Master_Data', master_file_path,
                ['<<POSITION_FOLDER>>', '<<POSITION_SHAPE>>', '<<STYLE_SHAPE>>'])

            upd_pf_tuple = def_common.make_position_folder_tuple(
                nsm_def.convert_array_to_tuple(bulk7['<<POSITION_FOLDER>>']),
                nsm_def.convert_array_to_tuple(bulk7['<<STYLE_SHAPE>>']),
                nsm_def.convert_array_to_tuple(bulk7['<<POSITION_SHAPE>>'])
            )
            nsm_def.write_sections_bulk(master_file_path, {
                '<<POSITION_FOLDER>>': upd_pf_tuple,
            })
            ...
            sync_time = 0.0
            if skip_sync:
                print('[Step 8/8] L2/L3 sync SKIPPED (deferred)')
            else:
                print('[Step 8/8] Syncing with L2/L3 layers...')
                ...
                if os.path.isfile(tmp_del):
                    os.remove(tmp_del)
                if tmp_del.lower().endswith('.nsm'):
                    tmp_xlsx = tmp_del[:-4] + '.xlsx'
                    if os.path.isfile(tmp_xlsx):
                        os.remove(tmp_xlsx)
    ...
    @staticmethod
    def update_device_location_batch(self, area_name, device_grid, master_file_path, skip_sync=False):
        """
        ★★★ OPTIMIZED BATCH VERSION ★★★
        ...
        try:
            ...
            is_nsm = str(master_file_path).lower().endswith('.nsm')
            wb = None
            if not is_nsm:
                import openpyxl
            ...
            # ★★★ BULK LOAD: Read all needed sections in ONE file open ★★★
            _bulk_sections = [
                '<<STYLE_FOLDER>>', '<<POSITION_SHAPE>>', '<<STYLE_SHAPE>>',
            ...
            # ★★★ Use pre-loaded bulk data instead of re-reading ★★★
            style_shape_array = _bulk['<<STYLE_SHAPE>>']
            position_tag_array = _bulk['<<POSITION_TAG>>']
            attribute_array = _bulk['<<ATTRIBUTE>>']
            ...
            for device_to_add in devices_to_add:
                ...
                text_dimensions = nsm_def.get_description_width_hight(shae_font_size, device_to_add)
                num_char_width = text_dimensions[0]
            ...
            if devices_to_delete:
                ...
                # POSITION_LINE
                position_line_array = _bulk['<<POSITION_LINE>>']
                header_rows = position_line_array[:2]
                data_rows = [item for item in position_line_array[2:]
                             if not (len(item) >= 2 and isinstance(item[1], list)
                                     and len(item[1]) >= 2
            ...
            # ========== BATCH STEP 4: Write ALL sections in ONE file open ==========
            print('[Info] Writing all changes (BULK)...')

            sections_to_write = {
                '<<POSITION_SHAPE>>': position_shape_array,
                '<<STYLE_SHAPE>>': style_shape_array,
                '<<POSITION_TAG>>': position_tag_array,
                '<<ATTRIBUTE>>': attribute_array,
            }
            if devices_to_delete:
                sections_to_write['<<POSITION_LINE>>'] = position_line_array

            nsm_def.write_sections_bulk(master_file_path, sections_to_write)
            ...
            _bulk5 = nsm_def.convert_master_to_arrays_bulk('Master_Data', master_file_path,
                ['<<POSITION_FOLDER>>', '<<POSITION_SHAPE>>', '<<STYLE_SHAPE>>'])
            position_folder_array = _bulk5['<<POSITION_FOLDER>>']
            position_shape_array = _bulk5['<<POSITION_SHAPE>>']
            style_shape_array = _bulk5['<<STYLE_SHAPE>>']
            ...
            nsm_def.remove_rows_under_section('Master_Data', master_file_path, ori_position_folder_tuple)
            nsm_def.write_excel_meta(update_position_folder_tuple, master_file_path,
            ...
            # ========== BATCH STEP 6: Sync with L2/L3 ==========
            if skip_sync:
                print('[Info] L2/L3 sync SKIPPED (deferred)')
            else:
                print('[Info] Syncing with L2/L3 layers...')
                import tkinter as tk
                dummy_tk = tk.Toplevel()
                dummy_tk.withdraw()
    ...
    def add_devices_for_new_areas(new_areas, master_file_path):
        ...
        try:
            # Parameters (from nsm_l1_master_create)
            ...
            for area_name in new_areas:
                ...
                text_dimensions = nsm_def.get_description_width_hight(shae_font_size, device_name)
                num_char_width = text_dimensions[0]
                line_width = max(num_char_width, width_min)
                line_height = height_min
    ...
    def update_area_with_map(self, area_location_array, master_file_path):
        ...
        old_position_folder_array = nsm_def.convert_master_to_array('Master_Data', master_file_path, '<<POSITION_FOLDER>>')
        old_area_positions = {}
        ...
        new_position_folder_array = nsm_def.convert_master_to_array('Master_Data', master_file_path, '<<POSITION_FOLDER>>')
        new_area_positions = {}
    ...
    def rebuild_position_folder(area_location_array, master_file_path):
        ...
        try:
            ...
            ori_position_folder_tuple = nsm_def.convert_array_to_tuple(position_folder_array)
            ...
            new_position_folder_tuple = nsm_def.convert_array_to_tuple(new_position_folder_array)

            # Write new POSITION_FOLDER data
            nsm_def.remove_rows_under_section('Master_Data', master_file_path, ori_position_folder_tuple)
            nsm_def.write_excel_meta(new_position_folder_tuple, master_file_path,
                                    'Master_Data', '<<POSITION_FOLDER>>', 0, 0)
    ...
    def recalculate_position_line_for_devices(moved_devices, master_file_path):
        ...
        position_shape_array = nsm_def.convert_master_to_array(ws_name, master_file_path, '<<POSITION_SHAPE>>')
        position_folder_array = nsm_def.convert_master_to_array(ws_name, master_file_path, '<<POSITION_FOLDER>>')
        position_line_array = nsm_def.convert_master_to_array(ws_name, master_file_path, '<<POSITION_LINE>>')
        ...
        if connections_updated > 0:
            position_line_tuple = nsm_def.convert_array_to_tuple(position_line_array)
            ori_position_line_tuple = nsm_def.convert_array_to_tuple(
                nsm_def.convert_master_to_array(ws_name, master_file_path, '<<POSITION_LINE>>')
            )
            nsm_def.remove_rows_under_section(ws_name, master_file_path, ori_position_line_tuple)
            nsm_def.write_excel_meta(position_line_tuple, master_file_path, ws_name, '<<POSITION_LINE>>', 0, 0)
    ...
    def recalculate_folder_sizes(master_file_path):
        ...
        try:
            ...
            # Write back
            cleaned_tuple = nsm_def.convert_array_to_tuple(position_folder_array_after)
            nsm_def.remove_rows_under_section('Master_Data', master_file_path,
                                             nsm_def.convert_array_to_tuple(nsm_def.convert_master_to_array('Master_Data', master_file_path, '<<POSITION_FOLDER>>')))
            nsm_def.write_excel_meta(cleaned_tuple, master_file_path,
                                    'Master_Data', '<<POSITION_FOLDER>>', 0, 0)
    ...
    def add_areas_to_style_folder(new_areas, master_file_path):
        ...
        try:
            ...
            new_style_folder_tuple = nsm_def.convert_array_to_tuple(style_folder_array)
            nsm_def.remove_rows_under_section('Master_Data', master_file_path, ori_style_folder_tuple)
            nsm_def.write_excel_meta(new_style_folder_tuple, master_file_path,
                                    'Master_Data', '<<STYLE_FOLDER>>', 0, 0)
    ...
    def add_waypoint_style_shape_tag(waypoint_name, master_file_path, direction=None):
        ...
        try:
            ...
            if not waypoint_exists_in_style:
                ...
                text_dimensions = nsm_def.get_description_width_hight(shae_font_size, waypoint_name)
                num_char_width = text_dimensions[0]
    ...
    def add_device_position_shape(device_name, reference_device, direction, position_shape_array, master_file_path):
        ...
        try:
            ...
            position_shape_tuple = nsm_def.convert_array_to_tuple(new_position_shape_array)

            # Remove existing rows under section before writing new data
            nsm_def.remove_rows_under_section('Master_Data', master_file_path, ori_position_shape_tuple)

            # Write new data
            nsm_def.write_excel_meta(position_shape_tuple, master_file_path,'Master_Data', '<<POSITION_SHAPE>>', 0, 0)
            ...
            if flag_wp_only:
                for tmp_tmp_master_folder_tuple in master_folder_tuple:
                    if tmp_master_width_size_y_grid[0] == tmp_tmp_master_folder_tuple[0] and tmp_tmp_master_folder_tuple[1] != 1:
                        if nsm_def.check_tuple_num_exist(update_master_folder_tuple, tmp_tmp_master_folder_tuple[0] - 1, tmp_tmp_master_folder_tuple[1] + 2):
    ...
    def process_l1_link_common(self, position_shape_array, style_shape_array, position_line_array,
        ...
        section_write_to = '<<POSITION_LINE>>'
        nsm_def.remove_rows_under_section('Master_Data', master_file_path, ori_position_line_tuple)
        nsm_def.write_excel_meta(self.position_line_tuple, master_file_path, 'Master_Data', section_write_to, offset_row, offset_column)

        # Write style_shape_array
        self.style_shape_array_tuple = nsm_def.convert_array_to_tuple(style_shape_array)
        section_write_to = '<<STYLE_SHAPE>>'
        nsm_def.overwrite_excel_meta(self.style_shape_array_tuple, excel_file_path, worksheet_name, section_write_to, offset_row, offset_column)
        ...
        self.position_tag_array_tuple = nsm_def.convert_array_to_tuple(position_tag_array)
        section_write_to = '<<POSITION_TAG>>'
        nsm_def.overwrite_excel_meta(self.position_tag_array_tuple, excel_file_path, worksheet_name, section_write_to, offset_row, offset_column)
        ...
        if not hasattr(self, 'inFileTxt_L3_1_1') or not self.inFileTxt_L3_1_1:
            dummy_tk = tk.Toplevel()
            dummy_tk.withdraw()
            self.inFileTxt_L3_1_1 = tk.Entry(dummy_tk)
            self.inFileTxt_L3_1_1.insert(tk.END, master_file_path)
