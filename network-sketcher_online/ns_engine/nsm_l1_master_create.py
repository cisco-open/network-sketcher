...
class  nsm_l1_master_create():
    def __init__(self):
        ...
        if self.click_value == '92-3':
            ...
            if nsm_def.check_file_open(str(self.inFileTxt_92_1.get()) + '__TMP__.xlsx') == True:
                return ()
        ...
        '''
        kari_master_folder_array = nsm_def.convert_tuple_to_array(master_folder_tuple)
        ...
        master_folder_tuple = nsm_def.convert_array_to_tuple(kari_master_folder_array)
        ...
        offset_row = 2
        nsm_def.write_excel_meta(master_style_folder_meta, self.excel_file_path, self.worksheet_name, write_to_section, offset_row, offset_column)
        offset_row = 0
        ...
        for each_folder_array in folder_array:
            ...
            for each_shape_array in shape_array:
                ...
                if flag_target_folder == True and (each_folder_array[0] != each_shape_array[0] or each_shape_array[1] == shape_array[-1][1]):
                    current_shape_tuple = nsm_def.return_shape_tuple(current_shape_array,start_row)
        ...
        for tmp_folder_name_of_wp in folder_name_of_wp:
            ...
            if flag_multi_wp == False:
                ...
                current_multi_wp_tuple = nsm_def.return_shape_tuple(tmp_sort_multi_wp, start_row)
        ...
        nsm_def.write_excel_meta(master_shape_tuple, self.excel_file_path, self.worksheet_name, write_to_section,offset_row, offset_column)
        ...
        for tmp_i_master_line_tuple in i_master_line_tuple:
            if nsm_def.check_tuple_num_exist(i_master_line_tuple, tmp_i_master_line_tuple[0], 1) == True or \
                    nsm_def.check_tuple_num_exist(i_master_line_tuple, tmp_i_master_line_tuple[0], 2) == True:
                if tmp_i_master_line_tuple[1] == 1 and i_master_line_tuple[tmp_i_master_line_tuple[0], 1] in wp_name_array:
                    ...
                    if nsm_def.check_tuple_num_exist(i_master_line_tuple, tmp_i_master_line_tuple[0], 7) == True:
                    ...
                    if nsm_def.check_tuple_num_exist(i_master_line_tuple, tmp_i_master_line_tuple[0], 8) == True:
                    ...
                    # MODIFIED: Only apply X-offset for UP/DOWN connections
                    if nsm_def.check_tuple_num_exist(i_master_line_tuple, tmp_i_master_line_tuple[0], 9) == True:
                        # Check if this line's direction is UP/DOWN (not LEFT/RIGHT)
                        is_updown = False
                    ...
                    if nsm_def.check_tuple_num_exist(i_master_line_tuple, tmp_i_master_line_tuple[0], 10) == True:
                        # Y-offset: keep original (not used for width)
        ...
        offset_row = 2
        nsm_def.write_excel_meta(master_line_tuple, self.excel_file_path, self.worksheet_name, write_to_section, offset_row, offset_column)
        offset_row = 0
        ...
        for tmp_master_udlr_array in master_udlr_array:
            ...
            num_char = nsm_def.get_description_width_hight(self.shae_font_size, hostname)[0]
        ...
        offset_row = 3
        nsm_def.write_excel_meta(master_style_shape_tuple, self.excel_file_path, self.worksheet_name, write_to_section, offset_row, offset_column)
        offset_row = 0
        ...
        if flag_put_line_tag == True:
            ...
            offset_row = 2
            nsm_def.write_excel_meta(master_line_tag_tuple, self.excel_file_path, self.worksheet_name, write_to_section, offset_row, offset_column)
            offset_row = 0
        ...
        #### GET best width size ####
        #### MODIFIED: Use common function from nsm_cli instead of duplicating code
        import nsm_cli

        # Call the common function
        update_master_folder_tuple = nsm_cli.def_common.make_position_folder_tuple(
            master_folder_tuple,
            master_style_shape_tuple,
            master_shape_tuple
        )
        ...
        offset_row = 0
        nsm_def.write_excel_meta(update_master_folder_tuple, self.excel_file_path, self.worksheet_name, write_to_section,offset_row, offset_column)
        ...
        nsm_def.write_excel_meta(master_root_folder_tuple, self.excel_file_path, self.worksheet_name, write_to_section, offset_row, offset_column)
        ...
        offset_row = 2
        nsm_def.write_excel_meta(master_attribute_tuple, self.excel_file_path, self.worksheet_name, write_to_section, offset_row, offset_column)
        offset_row = 0

if __name__ == '__main__':
    nsm_l1_master_create()
